...
public class ProductController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Product> updateProduct(@PathVariable("id") Long id, @RequestBody Product product) throws CrudOperationException, ProductNotSavedException {
        return new ResponseEntity<>(productService.updateProduct(id, product), HttpStatus.OK);
    }
    @PutMapping("/{id}")
    public ResponseEntity<Product> replaceProduct(@PathVariable("id") Long id, @RequestBody Product product) throws CrudOperationException, ProductNotSavedException {
        Product returnedProduct = productService.replaceProduct(id, product);
        return new ResponseEntity<>(returnedProduct, HttpStatus.OK);
    }
    @DeleteMapping("/{id}")
    public ResponseEntity<Product> deleteProduct(@PathVariable("id") Long id) throws CrudOperationException {
